(() => {
  ...
  const setStorageJson = (key, value) => localStorage.setItem(key, JSON.stringify(value));
  ...
  const setLocalFavorites = (favorites) => setStorageJson(STORAGE_KEYS.FAVORITES, favorites);
  const getLocalSavedSearches = () => getStorageJson(STORAGE_KEYS.SAVED_SEARCHES, []);
  const setLocalSavedSearches = (savedSearches) => setStorageJson(STORAGE_KEYS.SAVED_SEARCHES, savedSearches);
  const getLocalReports = () => getStorageJson(STORAGE_KEYS.REPORTS, []);
  const setLocalReports = (reports) => setStorageJson(STORAGE_KEYS.REPORTS, reports);
  ...
  const normalizeMockRoom = (room) => {
    const favoriteIds = new Set(getLocalFavorites().map((item) => Number(item.roomId)));
    const reports = getLocalReports();
    return {
      ...room,
      images: Array.isArray(room.images) ? room.images.map(resolveImageUrl) : [],
      ...
      favoriteCount: getLocalFavorites().filter((item) => Number(item.roomId) === Number(room.id)).length,
      reportCount: reports.filter((item) => Number(item.roomId) === Number(room.id)).length
    };
  };

  const normalizeRoomImages = (room) => {
    if (!room) return room;
    const normalizedImages = Array.isArray(room.images) ? room.images.map(resolveImageUrl) : [];
    return {
      ...room,
  ...
  const filterRooms = (rooms, params = {}) => {
    ...
    const district = String(params.district || '').toLowerCase().trim();
    ...
    return rooms.filter((room) => {
      ...
      const matchesKeyword =
        ...
        String(room.address || '').toLowerCase().includes(keyword) ||
        String(room.city || '').toLowerCase().includes(keyword);
      const matchesDistrict = !district || String(room.district || '').toLowerCase().includes(district);
  ...
  const getRooms = async (params = {}) => {
    if (USE_MOCK) {
      const rooms = filterRooms(getAllMockRooms().map(normalizeMockRoom), params);
    ...
    return request(`/rooms/search?${query.toString()}`, { optionalAuth: true }).then(normalizeRoomPage);
  };
  ...
  const getInitialRooms = async (limit = 5) => {
    ...
    return request(`/rooms/initial?limit=${Number(limit) || 5}`, { optionalAuth: true }).then(normalizeRoomPage);
  };
  ...
  const getFeaturedRooms = async (limit = 12) => {
    ...
    return request(`/rooms/highlights?limit=${Number(limit) || 12}`, { optionalAuth: true }).then(normalizeRoomPage);
  };
  ...
  const getRoomById = async (id) => {
    ...
    return request(`/rooms/${id}`, { optionalAuth: true }).then(normalizeRoomImages);
  };
  ...
  const register = async (payload) => {
    if (USE_MOCK) return { success: true, message: 'Đăng ký thành công (mock).' };
  ...
  const getUserProfile = async (id) => {
    if (USE_MOCK) {
      const currentUser = getCurrentUser();
      if (currentUser && Number(currentUser.id) === Number(id)) return currentUser;
  ...
  const changePassword = async (id, oldPassword, newPassword) => {
    if (USE_MOCK) return { success: true, message: 'Đã đổi mật khẩu thành công (mock)' };
  ...
  const getFirst12RoomIds = async () => {
    if (USE_MOCK) return getAllMockRooms().map((room) => Number(room.id)).slice(0, 12);
  ...
  const getMyRooms = async () => {
    if (USE_MOCK) {
      const currentUser = getCurrentUser();
      const rooms = getLocalRooms().filter((room) => Number(room.userId) === Number(currentUser?.id)).map(normalizeMockRoom);
      return normalizeRoomPage({ content: rooms, totalElements: rooms.length });
    }
  ...
  const updateRoom = async (id, roomPayload, imageFiles = []) => {
    if (USE_MOCK) {
      ...
      localRooms[index] = {
        ...localRooms[index],
        ...roomPayload,
        images: imageFiles.length ? imageFiles.map((file) => URL.createObjectURL(file)) : localRooms[index].images
      };
      setLocalRooms(localRooms);
  ...
  const deleteRoom = async (id) => {
    if (USE_MOCK) {
      setLocalRooms(getLocalRooms().filter((room) => Number(room.id) !== Number(id)));
      setLocalFavorites(getLocalFavorites().filter((item) => Number(item.roomId) !== Number(id)));
      setLocalReports(getLocalReports().filter((item) => Number(item.roomId) !== Number(id)));
      return true;
    }
  ...
  const getAllRoomsForAdmin = async () => {
    if (USE_MOCK) {
      const rooms = getAllMockRooms().map(normalizeMockRoom).filter((room) => String(room.status || '').toUpperCase() === 'PENDING');
  ...
  const getOwnerStats = async () => {
    if (USE_MOCK) {
      ...
      return {
        ...
        totalViews: rooms.reduce((sum, room) => sum + Number(room.viewCount || 0), 0),
        totalContactClicks: rooms.reduce((sum, room) => sum + Number(room.contactClickCount || 0), 0),
        totalFavorites: rooms.reduce((sum, room) => sum + Number(room.favoriteCount || 0), 0)
      };
    }
  ...
  const addFavorite = async (roomId) => {
    if (USE_MOCK) {
      ...
      if (!favorites.some((item) => Number(item.roomId) === Number(roomId) && Number(item.userId) === Number(currentUser?.id))) {
        favorites.push({ id: Date.now(), userId: currentUser?.id, roomId: Number(roomId), createdAt: new Date().toISOString() });
  ...
  const removeFavorite = async (roomId) => {
    if (USE_MOCK) {
      const currentUser = getCurrentUser();
      setLocalFavorites(
        getLocalFavorites().filter((item) => !(Number(item.roomId) === Number(roomId) && Number(item.userId) === Number(currentUser?.id)))
      );
      return { success: true };
    }
    return request(`/rooms/${roomId}/favorite`, { method: 'DELETE', auth: true });
  };
  ...
  const getMyFavorites = async () => {
    if (USE_MOCK) {
      ...
      const rooms = getAllMockRooms().filter((room) => ids.has(Number(room.id))).map(normalizeMockRoom);
      return normalizeRoomPage({ content: rooms, totalElements: rooms.length });
    }
    return request('/rooms/favorites/my', { auth: true }).then(normalizeRoomPage);
  };
  ...
  const saveSearch = async (payload) => {
    if (USE_MOCK) {
      ...
      const saved = { id: Date.now(), userId: currentUser?.id, createdAt: new Date().toISOString(), ...payload };
  ...
  const getSavedSearches = async () => {
    if (USE_MOCK) {
      const currentUser = getCurrentUser();
      return getLocalSavedSearches().filter((item) => Number(item.userId) === Number(currentUser?.id));
    }
  ...
  const deleteSavedSearch = async (id) => {
    if (USE_MOCK) {
      setLocalSavedSearches(getLocalSavedSearches().filter((item) => Number(item.id) !== Number(id)));
      return { success: true };
    }
    return request(`/rooms/saved-searches/${id}`, { method: 'DELETE', auth: true });
  };

  const reportRoom = async (roomId, payload) => {
    if (USE_MOCK) {
      const currentUser = getCurrentUser();
      const room = getAllMockRooms().find((item) => Number(item.id) === Number(roomId));
      const reports = getLocalReports();
      const saved = {
        id: Date.now(),
  ...
  const recordContactClick = async (roomId) => {
    if (USE_MOCK) {
      const localRooms = getLocalRooms();
      const index = localRooms.findIndex((room) => Number(room.id) === Number(roomId));
      if (index !== -1) {
        localRooms[index] = {
          ...localRooms[index],
          contactClickCount: Number(localRooms[index].contactClickCount || 0) + 1
        };
        setLocalRooms(localRooms);
      }
      return { success: true };
